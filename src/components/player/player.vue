<template>
  <div class="player" v-show="playList.length>0" @click="hideBar">
      ...
      <div class="normal-player" @click="hidePlaylist" v-show="fullScreen">
        ...
        <div class="top">
          ...
          <div class="volume" id="volume">
            <i @click.stop="toggleShowBar" class="v-icon fa" :class="volumIconShow"></i><i v-show="silence" :class="greyText" class="text">×</i>
            <progress-bar class="volume-bar" v-show="showBar && hasSetShow" @percentChange="changeVolume" :volumeFlag="volumeFlag" :percent="volumePercent"></progress-bar>
          </div>
        </div>
        <div class="middle"
        ...
        <div class="bottom">
          ...
          <div class="options">
            ...
            <div class="icon i-left" :class="disableCls">
              <i @click="downloadSong" class="fa fa-download"></i>
            </div>
            <div class="icon i-left" :class="disableCls">
              <i @click="showComment" class="fa fa-commenting"></i>
            </div>
            <div class="icon i-left">
...
const CIRCLE_BTN_WIDTH = 16
const MARGIN_LEFT = 20
const DEFAULT_VOLUME = document.documentElement.clientWidth - CIRCLE_BTN_WIDTH - MARGIN_LEFT || document.body.clientWidth - CIRCLE_BTN_WIDTH - MARGIN_LEFT
export default {
  mixins: [playerModeMixin],
  data() {
    return {
      showCommentFlag: false, // 是否显示评论
      video: false,
      ...
      hasSetShow: false, // 显示音量条
      volumeFlag: true, // 是否有音量条
      showVolumeBar: false, // 默认不显示
      radius: 32, // 圆形播放进度条半径
  ...
  computed: {
    greyText() {
      return this.currentVolume === 0 && !this.hasSetShow ? 'grey' : ''
    },
    volumIconShow() {
      let offFlag1 = !this.hasSetShow
      let offFlag2 = this.silence
      return offFlag1 ? 'fa-volume-off grey' : offFlag2 ? 'fa-volume-off' : 'fa-volume-up'
    },
    // 禁用播放按钮
    disableCls() {
  ...
  methods: {
    ...
    loadMore() {
      ...
      if (!this.hasMore) {
        if (this.firstFlag) {
          this.$refs.comment.resetHeight(80)
      ...
      // 加载下一页
      this.pagenum++
      this.cmd = this.currentCommentIndex === 0 ? 6 : 8
      this.pagesize = this.currentCommentIndex === 0 ? 10 : 25
      getComments(this.cmd, this.pagenum, this.pagesize, this.currentSong.id).then((res) => {
        this.comment = res.comment.commenttotal > 0 ? this.comment.concat(res.comment.commentlist) : []
        // 检测是否存在下一页
        this._checkMore(res)
    ...
    _checkMore(res) {
      this.hasMore = res.morecomment === 1
    },
    hideComment() {
    ...
    showComment() {
      this.initComment()
      getComments(cmd, pagenum, pagesize, this.currentSong.id).then((res) => {
        // 获取热门评论
        if (this.currentCommentIndex === 0) {
          this.title = '暂无热评'
          // 热评数量少于pagesize
          if (res.hot_comment.commenttotal <= pagesize) {
            this.hasMore = false
          }
          // 存在热评
          if (res.hot_comment.commenttotal > 0) {
            this.comment = res.hot_comment.commentlist
          } else {
            this.noResult = true
          }
        } else {
          this.title = '暂无评论'
          // 评论数量少于pagesize
          if (res.comment.commenttotal <= pagesize) {
            this.hasMore = false
          }
          // 存在最新评论
          if (res.comment.commenttotal > 0) {
            this.comment = res.comment.commentlist
          } else {
            this.noResult = true
          }
        }
    ...
    downloadSong() {
      ...
      this.downloadFunc('http://isure.stream.qqmusic.qq.com/C400001uqhf00utqzP.m4a?vkey=6F80B510CA860F092542E280416B047A90514291B74C90C8A58CCBFE358AB2A03F2CA89AB615C825A7636A7AF708C26E8DD99D00DF0AD9CC&guid=2238432363&uin=2733358560&fromtag=66', `${this.currentSong.name}.mp3`)
    },
    hideBar() {
      this.hasSetShow = false
    },
    // 是否显示音量控制条
    toggleShowBar() {
      this.hasSetShow = !this.hasSetShow
    },
    // 显示播放列表
    showPlaylist() {
  ...
  watch: {
    ...
    currentSong(newSong, oldSong) {
      ...
      setTimeout(() => {
        this.$refs.audio.play()
        document.getElementById('volume').style.width = DEFAULT_VOLUME + 'px'
